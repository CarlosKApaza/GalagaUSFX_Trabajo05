...
ANaveEnemigaCazador::ANaveEnemigaCazador()
{
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> ShipMesh(TEXT("StaticMesh'/Game/TwinStick/Meshes/NaveCaza.NaveCaza'"));
...
void ANaveEnemigaCazador::Move(float DeltaTime)
{
...
void ANaveEnemigaCazador::Shoot(FVector FireDirection)
{
	...
	if (bCanFire == true)
	{
		...
		if (FireDirection.SizeSquared() > 0.0f)
		{
			...
			bCanFire = false;
			World->GetTimerManager().SetTimer(TimerHandle_ShotTimerExpired, this, &ANaveEnemigaCazador::ShotTimerExpired, FireRate);
			bCanFire = false;
		}
	}
}

void ANaveEnemigaCazador::RecibirDanio()
{
	Energia -= 5;
	if (Energia <= 0)
	{
...
void ANaveEnemigaCazador::Curarse()
{
	Energia = 100;
}

void ANaveEnemigaCazador::ShotTimerExpired()
{
	bCanFire = true;
}